...
# Temporary deterministic fallback for known false-negative Viega drain-body pages.
# Intentionally narrow: only these IDs are overridden. New/future products still use
# generic logic and are unaffected. Remove when generic promotion is reliable.
...
def run_update(
    ...
    viega_debug = {
        ...
        "sample_overridden_rows": [],
        "why_not_product_reason_counts": {},
    }
    ...
    for _, r in registry_df.iterrows():
        ...
        if manufacturer == "viega":
            ...
            if explicit_override:
                ...
                if len(viega_debug["sample_overridden_rows"]) < 20:
                    viega_debug["sample_overridden_rows"].append(f"{product_id} => {url}")
        ...
        if manufacturer == "viega":
            why = "" if promote_to_product else promotion_reason
            viega_debug["why_not_product_reason_counts"][why] = viega_debug["why_not_product_reason_counts"].get(why, 0) + 1
        if manufacturer == "viega":
    ...
    if any(str(x).strip().lower() == "viega" for x in registry_df.get("manufacturer", pd.Series(dtype=str)).tolist()):
        ...
        evidence_rows.append({
            "manufacturer": "viega",
            "product_id": "__summary__",
            "label": "why_not_product_reason_counts",
            "snippet": str(viega_debug["why_not_product_reason_counts"]),
            "source": "promotion_stage",
        })
        evidence_rows.append({
            "manufacturer": "viega",
        ...
        evidence_rows.append({
            "manufacturer": "viega",
            "product_id": "__summary__",
            "label": "sample_overridden_rows",
            "snippet": str(viega_debug["sample_overridden_rows"][:10]),
            "source": "promotion_stage",
        })
        evidence_rows.append({
            "manufacturer": "viega",
