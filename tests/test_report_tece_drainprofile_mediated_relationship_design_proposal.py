...
def _valid_evidence_rows():
    ...
    for i,(article,area,target) in enumerate(zip(validator.EXPECTED_ARTICLE_ORDER,areas,targets),1):
        ...
        row.update({"template_v2_row_id":f"{validator.ROW_ID_PREFIX}-{i:06d}","family":validator.EXPECTED_FAMILY,"evidence_collection_area":area,"evidence_target_type":target,"article_number":article,"source_document_name":"TECE catalogue","source_document_version":"2024","source_page_or_section":"p.1","source_url_or_path":"https://example.test/tece","source_text_excerpt":"Official diagnostic scope evidence only.","reviewed_evidence_summary":f"Official TECE evidence for {article or 'Duschprofil scope'}.","reviewer_decision":next(iter(validator.ALLOWED_DECISIONS[area])),"reviewer_notes":notes,"safe_to_use_for_future_diagnostic_design":"true","diagnostic_only":"true"})
...
def artifacts(tmp_path: Path, fake_source_pack):
    a=SimpleNamespace()
    a.validation=tmp_path/'tecedrainprofile_mediated_relationship_evidence_template_v2_REVIEWED_validation.json'
    a.template=tmp_path/'tecedrainprofile_mediated_relationship_evidence_template_v2_report.json'
    a.evidence=tmp_path/'tecedrainprofile_mediated_relationship_evidence_template_v2_REVIEWED.csv'
    a.qa_report=tmp_path/'tecedrainprofile_mediated_relationship_evidence_template_v2_QA_report.json'
    a.qa_csv=tmp_path/'tecedrainprofile_mediated_relationship_evidence_template_v2_QA.csv'
    a.source_pack=tmp_path/'source_pack'; a.source_pack.mkdir(); (a.source_pack/'keep.txt').write_text('unchanged')
    a.validation.write_text(json.dumps({'valid': True, 'errors': []}))
    a.template.write_text(json.dumps({'valid': True}))
    a.qa_report.write_text(json.dumps({'valid': True, 'future_diagnostic_design_ready_count': 15}))
    _write_csv(a.evidence, _valid_evidence_rows())
    _write_csv(a.qa_csv, [{'qa_row_id': f'QA-{i:06d}', 'qa_status': 'pass'} for i in range(1,16)])
    a.out=tmp_path/'proposal.csv'; a.json_out=tmp_path/'proposal.json'
    return a

def _report(a): return mod.generate_report(a.qa_report,a.qa_csv,a.validation,a.evidence,a.template,a.source_pack)
def _mut_csv(a, fn):
    rows=list(csv.DictReader(a.evidence.open(encoding='utf-8-sig'))); fn(rows); _write_csv(a.evidence, rows)
...
def test_cli_writes_csv_and_json(artifacts, capsys):
    rc=mod.main(['--qa-report',str(artifacts.qa_report),'--qa-csv',str(artifacts.qa_csv),'--validation-report',str(artifacts.validation),'--evidence-csv',str(artifacts.evidence),'--template-v2-report',str(artifacts.template),'--source-pack',str(artifacts.source_pack),'--out',str(artifacts.out),'--json-out',str(artifacts.json_out),'--json'])
...
def test_wrong_family_fails(artifacts): assert mod.generate_report(artifacts.qa_report,artifacts.qa_csv,artifacts.validation,artifacts.evidence,artifacts.template,artifacts.source_pack,'Wrong')[0]['valid'] is False
...
def test_input_row_count_mismatch_fails(artifacts, target):
    path=getattr(artifacts, target); rows=list(csv.DictReader(path.open(encoding='utf-8-sig'))); rows.pop(); _write_csv(path, rows)
    assert _report(artifacts)[0]['valid'] is False
...
@pytest.mark.parametrize('name,value', [('RETAINED_DRAIN_BODY_ARTICLES',['673001']),('PROPOSED_PROFILE_COVER_ARTICLES',['675000'])])
...
@pytest.mark.parametrize('area', ['mediated_relationship_topology','direct_pairing_model_rejection'])
def test_mediated_and_direct_rejection_enforced(artifacts, monkeypatch, area):
    ...
    def bad(family=mod.EXPECTED_FAMILY):
        rows=orig(family)
        for r in rows:
            if r['proposal_area']==area: r['relationship_model']='broken'
        return rows
    monkeypatch.setattr(mod, 'build_design_proposal_rows', bad)
    assert _report(artifacts)[0]['valid'] is False

def test_direct_pairing_claim_leakage_fails(artifacts):
    _mut_csv(artifacts, lambda rows: rows[0].update(reviewer_notes='675001 is compatible with 673001'))
    assert _report(artifacts)[0]['valid'] is False

@pytest.mark.parametrize('field', mod.FALSE_ALLOWED_FIELDS)
def test_generation_mutation_promotion_readiness_flags_fail(artifacts, monkeypatch, field):
    orig=mod.build_design_proposal_rows
    def bad(family=mod.EXPECTED_FAMILY):
        rows=orig(family); rows[0][field]='true'; return rows
    monkeypatch.setattr(mod, 'build_design_proposal_rows', bad)
...
@pytest.mark.parametrize('key,value', [('production_promotion_blocked', False), ('ready_for_benchmark', True), ('ready_for_customer_view', True)])
def test_fixed_production_and_readiness_values(artifacts, key, value, monkeypatch):
    report,_=_report(artifacts)
    assert report[key] is (not value if key=='production_promotion_blocked' else False)

def test_diagnostic_only_false_fails(artifacts, monkeypatch):
    orig=mod.build_design_proposal_rows
    def bad(family=mod.EXPECTED_FAMILY):
        rows=orig(family); rows[0]['diagnostic_only']='false'; return rows
    monkeypatch.setattr(mod, 'build_design_proposal_rows', bad)
    assert _report(artifacts)[0]['valid'] is False

def test_source_pack_input_immutability(artifacts):
    files=[artifacts.validation,artifacts.evidence,artifacts.template,artifacts.qa_report,artifacts.qa_csv,artifacts.source_pack/'keep.txt']
    before={p:hashlib.sha256(p.read_bytes()).hexdigest() for p in files}
    mod.main(['--qa-report',str(artifacts.qa_report),'--qa-csv',str(artifacts.qa_csv),'--validation-report',str(artifacts.validation),'--evidence-csv',str(artifacts.evidence),'--template-v2-report',str(artifacts.template),'--source-pack',str(artifacts.source_pack),'--out',str(artifacts.out),'--json-out',str(artifacts.json_out)])
    assert before=={p:hashlib.sha256(p.read_bytes()).hexdigest() for p in files}

def test_diagnostic_only_note_present(artifacts): assert _report(artifacts)[0]['diagnostic_only_note']==mod.DIAGNOSTIC_ONLY_NOTE
