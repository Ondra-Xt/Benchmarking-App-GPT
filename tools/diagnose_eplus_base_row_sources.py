...
HEIGHT_RANGE_RE = re.compile(
    r"(?:einbauh(?:ö|oe)he|oberkante\s+estrich|height)[^\d]{0,50}(?P<min>\d{1,3})\s*[-–]\s*(?P<max>\d{1,3})\s*mm",
    re.IGNORECASE,
)
...
SHOWERDRAIN_C_RE = re.compile(r"(?:aco\s+)?showerdrain\s+c\b|showerdrain-c\b", re.IGNORECASE)
SUPPORT_RE = re.compile(r"montagewinkel|mounting|support|stütz|stuetz|befestigung(?:smaterial)?", re.IGNORECASE)
GRATE_RE = re.compile(r"design[-\s]?roste?|designrost|\broste?\b|\brost\b|grate", re.IGNORECASE)
ACCESSORY_RE = re.compile(r"zubeh(?:ö|oe)r|accessor|adapter|haarsieb|sieb|showerstep|rahmen|dicht|flansch|schallschutz", re.IGNORECASE)
BODY_RE = re.compile(r"rinnenk(?:ö|oe)rper|einbauh(?:ö|oe)he|ablaufk(?:ö|oe)rper|ablaufleistung|sperrwasserh(?:ö|oe)he|\bdn\b", re.IGNORECASE)
...
class EPlusReadinessSummary:
    ...
    eplus_grate_components: int
    eplus_accessories: int
    excluded_cross_family: int
    excluded_support_or_mounting_part: int
    excluded_other: int
...
class EPlusDiagnostic:
    ...
    grate_components: tuple[EPlusCandidateRow, ...]
    accessories: tuple[EPlusCandidateRow, ...]
    excluded_cross_family: tuple[EPlusCandidateRow, ...]
    excluded_support_or_mounting_part: tuple[EPlusCandidateRow, ...]
    excluded_other: tuple[EPlusCandidateRow, ...]
...
def _is_page_level_body(source_url: str, title: str) -> bool:
    page_text = f"{source_url} {title}"
    return bool(EPLUS_RE.search(page_text) and BODY_RE.search(page_text) and not GRATE_RE.search(page_text))


def _references_showerdrain_c_only(row_text: str, compatible_with: str = "") -> bool:
    text = f"{row_text} {compatible_with}"
    return bool(SHOWERDRAIN_C_RE.search(text) and not EPLUS_RE.search(text))


def _classify_candidate_type(source_url: str, title: str, row_text: str, compatible_with: str) -> tuple[str, str, str, str]:
    page_text = f"{source_url} {title}"
    text = f"{page_text} {row_text}"
    if SUPPORT_RE.search(row_text):
        return "excluded_support_or_mounting_part", "component", "support_or_mounting_part", "high"
    if _references_showerdrain_c_only(row_text, compatible_with):
        return "excluded_cross_family", "", "", "high"
    if GRATE_RE.search(text):
        return "eplus_grate_component", "component", "grate", "high" if GRATE_RE.search(page_text) else "medium"
    if ACCESSORY_RE.search(row_text):
        return "eplus_accessory", "component", "accessory", "medium"
    if _is_page_level_body(source_url, title):
        return "eplus_article_level_body_candidate", "drain", "drain_unit", "medium"
    if EPLUS_RE.search(row_text) and BODY_RE.search(row_text):
        return "eplus_article_level_body_candidate", "drain", "drain_unit", "medium"
    if EPLUS_RE.search(row_text):
...
def _extract_compat(row_text: str) -> str:
    ...
    if not matches:
        return ""
    with_family = [match for match in matches if EPLUS_RE.search(match) or SHOWERDRAIN_C_RE.search(match)]
...
def _candidate_from_text(source_url: str, title: str, row_text: str, evidence_type: str) -> EPlusCandidateRow | None:
    ...
    candidate_type, _base_type, system_role, confidence = _classify_candidate_type(source_url, title, cleaned, compatible_with)
    ...
    proposed_product_id = ""
    if article_digits and candidate_type not in {"excluded_cross_family", "excluded_support_or_mounting_part", "excluded_other"}:
    ...
    row = EPlusCandidateRow(
        ...
        product_family=EPLUS_FAMILY if candidate_type.startswith("eplus_") else "",
    ...
    if row.candidate_type == "eplus_article_level_body_candidate" and not _has_complete_hydraulics(row):
        row = EPlusCandidateRow(**{**row.__dict__, "candidate_type": "eplus_article_level_body_incomplete", "confidence": "medium"})
    return row


def _page_level_body_evidence(source_url: str, title: str, flat_text: str) -> EPlusCandidateRow | None:
    if not _is_page_level_body(source_url, title):
        return None
    product_id = aco._stable_aco_id(source_url, EPLUS_FAMILY, "drain_unit", title)
    height_min, height_max = _extract_height(f"{title} {flat_text[:700]}")
    return EPlusCandidateRow(
        ...
        source_url=source_url,
        row_text=_clean_text(f"{title} {flat_text[:500]}"),
        flow_rate_lps=_first_match(FLOW_RE, flat_text),
        water_seal_mm=_first_match(SEAL_RE, flat_text),
        outlet_dn=f"DN{_first_match(DN_RE, flat_text)}" if _first_match(DN_RE, flat_text) else "",
        height_adj_min_mm=height_min,
        height_adj_max_mm=height_max,
        evidence_type="source_page_level_body",
        confidence="high",
    )
...
def _table_row_texts(html: str) -> list[str]:
    ...
    for table in soup.select("table"):
        ...
        for tr in table.select("tr"):
            ...
            if ARTICLE_RE.search(row_text) or EPLUS_RE.search(row_text) or SHOWERDRAIN_C_RE.search(row_text):
...
def parse_eplus_source_url(source_url: str) -> tuple[str, tuple[EPlusCandidateRow, ...]]:
    ...
    page_evidence = _page_level_body_evidence(final_url, title, flat_text)
    if page_evidence is not None:
        candidates.append(page_evidence)
    ...
    if not raw_rows and EPLUS_RE.search(f"{final_url} {title} {flat_text}") and page_evidence is None:
        raw_rows = [flat_text[:700]]
        evidence_type = "source_page_text"

    seen_keys: set[tuple[str, str, str]] = set()
    for candidate in candidates:
        seen_keys.add((candidate.article_number, candidate.source_url, candidate.candidate_type))
    for raw_row in raw_rows:
        candidate = _candidate_from_text(final_url, title, raw_row, evidence_type)
        if candidate is None:
            continue
        key = (candidate.article_number, candidate.source_url, candidate.candidate_type)
        if key in seen_keys:
            continue
        seen_keys.add(key)
...
def parse_eplus_sources(source_urls: Iterable[str]) -> tuple[tuple[str, ...], tuple[EPlusCandidateRow, ...]]:
    ...
    for url in source_urls:
        ...
        for row in rows:
            key = (row.article_number, row.source_url, row.candidate_type)
            if key in seen_keys:
                continue
            seen_keys.add(key)
...
def build_readiness_summary(output_rows: OutputRowsSummary, rows: tuple[EPlusCandidateRow, ...]) -> EPlusReadinessSummary:
    page_body = tuple(row for row in rows if row.candidate_type == "eplus_page_level_body_evidence")
    article_body = tuple(row for row in rows if row.candidate_type == "eplus_article_level_body_candidate")
    incomplete = tuple(row for row in rows if row.candidate_type == "eplus_article_level_body_incomplete")
    ...
    cross = tuple(row for row in rows if row.candidate_type == "excluded_cross_family")
    support = tuple(row for row in rows if row.candidate_type == "excluded_support_or_mounting_part")
    other = tuple(row for row in rows if row.candidate_type == "excluded_other")
    complete = tuple(row for row in article_body if _has_complete_hydraulics(row))

    model = "base_x_grate" if page_body and grates else "unknown"
    blocking = "E+ source-backed page-level body/grate evidence exists, but article-level hydraulic fields are incomplete and cross-family/support rows are excluded."
    next_action = "Collect source-backed article-level E+ base/body hydraulic fields and explicit base-to-grate compatibility before proposing any production generation."
    ...
    return EPlusReadinessSummary(
        ...
        eplus_page_level_body_rows=len(page_body),
        eplus_article_level_body_candidates=len(article_body),
        eplus_article_level_body_incomplete=len(incomplete),
        eplus_grate_components=len(grates),
        eplus_accessories=len(accessories),
        excluded_cross_family=len(cross),
        excluded_support_or_mounting_part=len(support),
...
def build_diagnostic(
    ...
    return EPlusDiagnostic(
        ...
        article_level_body_incomplete=tuple(row for row in source_candidates if row.candidate_type == "eplus_article_level_body_incomplete"),
        grate_components=tuple(row for row in source_candidates if row.candidate_type == "eplus_grate_component"),
        accessories=tuple(row for row in source_candidates if row.candidate_type == "eplus_accessory"),
        excluded_cross_family=tuple(row for row in source_candidates if row.candidate_type == "excluded_cross_family"),
        ...
        excluded_other=tuple(row for row in source_candidates if row.candidate_type == "excluded_other"),
        readiness=readiness,
    )
...
def print_report(diag: EPlusDiagnostic) -> None:
    print("ACO ShowerDrain E+ base/body source investigation diagnostic (read-only; no generation)")
    ...
    _print_candidates("Grates", diag.grate_components)
    _print_candidates("Accessories", diag.accessories)
    _print_candidates("Excluded cross-family rows", diag.excluded_cross_family)
    _print_candidates("Excluded support/accessory rows", diag.excluded_support_or_mounting_part)
    if diag.excluded_other:
        _print_candidates("Excluded other", diag.excluded_other)
    ...
    print(f"- excluded cross-family rows: {r.excluded_cross_family}")
    print(f"- excluded support/accessory rows: {r.excluded_support_or_mounting_part + r.eplus_accessories}")
    print(f"- grates: {r.eplus_grate_components}")
    print(f"- hydraulic_complete_candidates: {r.hydraulic_complete_candidates}")
    print(f"- likely_assembly_model: {r.likely_assembly_model}")
