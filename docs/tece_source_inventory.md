...
TECEdrainline `Designrost` tables with columns such as `Nennlänge`, `Oberfläche`, `Best.-Nr.`, and `LE 1` require strict same-row or same-index attribution from the product table: each `Best.-Nr.` must receive its own `Nennlänge` and surface value. The `LE 1` value is a packaging/logistics field and must not be used as a dimension, and the first row's `Nennlänge` must not be carried forward to later article numbers.
