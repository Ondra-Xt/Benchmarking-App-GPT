...
def validate_export(xlsx_path: Path) -> None:
    ...
    # Global height sanity checks (Products + Components)
    for sheet_name, df in [("Products", products), ("Components", components)]:
        if "height_adj_min_mm" not in df.columns or "height_adj_max_mm" not in df.columns:
            continue
        ...
        suspicious_low = df[df["height_adj_max_mm"].notna() & (df["height_adj_max_mm"] <= 30)]
        if not suspicious_low.empty:
            warn(
                f"{sheet_name} suspicious height <= 30 mm: "
                f"{suspicious_low[height_cols].head(50).to_dict(orient='records')}"
            )

        suspicious_mixed = df[
            df["height_adj_min_mm"].notna()
            & df["height_adj_max_mm"].notna()
            & (df["height_adj_min_mm"] <= 20)
            & (df["height_adj_max_mm"] >= 50)
        ]
        if not suspicious_mixed.empty:
            warn(
                f"{sheet_name} suspicious mixed height (tile depth + install height): "
                f"{suspicious_mixed[height_cols].head(50).to_dict(orient='records')}"
            )
        ...
        if not missing_height.empty:
            warn(
                ...
                f"{missing_height[height_cols].head(50).to_dict(orient='records')}"
            )
